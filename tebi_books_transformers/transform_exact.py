...
def build_exact_csv(df, admin_code, journal_code, differences_ledger, currency="EUR", cost_center_code=None, journal_type="KAS", round_tolerance=Decimal("0.05")):
    ...
    Returns:
        dict: Dictionary mapping (year, month) tuples to CSV bytes
              e.g., {(2023, 11): bytes, (2023, 12): bytes}
    """
    ...
    # Dictionary to store CSV bytes per month
    monthly_csvs = {}
    ...
    for (year, month), month_df in df.groupby([df["Date"].dt.year, df["Date"].dt.month]):
        ...
        # Group by date to create document numbers and balance per day within this month
        for date_val, group in month_df.groupby(month_df["Date"].dt.date):
            if pd.isna(date_val):
                continue
        ...
        # Create DataFrame with proper column order for this month
        ...
        # Store this month's CSV
        monthly_csvs[(int(year), int(month))] = mem.getvalue()
